...
function subscriptionOptions(
  ...
  tag: string,
  retryNonTransportErrors?: boolean,
): StreamSubscriptionOptions & { readonly tag: string } {
  return {
    ...options,
    tag,
    ...(retryNonTransportErrors === undefined ? {} : { retryNonTransportErrors }),
  };
}
    ...
    orchestration: {
      ...
      subscribeShell: (listener, options) =>
        transport.subscribe(
          ...
          listener,
          subscriptionOptions(options, ORCHESTRATION_WS_METHODS.subscribeShell, true),
        ),
      subscribeThread: (input, listener, options) =>
        transport.subscribe(
          (client) => client[ORCHESTRATION_WS_METHODS.subscribeThread](input),
          listener,
          subscriptionOptions(options, ORCHESTRATION_WS_METHODS.subscribeThread, true),
        ),
    },
  };
}
