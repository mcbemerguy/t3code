...
export function createEnvironmentConnection(
  ...
  let disposed = false;
  let hasShellSnapshot = false;
  let recoverAfterNextShellSnapshot = false;
  let shellResubscribeGeneration = 0;
  let shellSnapshotAfterResubscribeGeneration = 0;
  let recoveryChain: Promise<void> = Promise.resolve();
  const bootstrapGate = createBootstrapGate();
  const shellSnapshotAfterResubscribeWaiters = new Set<{
  ...
  const resolveShellSnapshotAfterResubscribeWaiters = () => {
    for (const waiter of shellSnapshotAfterResubscribeWaiters) {
      if (shellSnapshotAfterResubscribeGeneration <= waiter.afterGeneration) {
        continue;
      }

      shellSnapshotAfterResubscribeWaiters.delete(waiter);
      waiter.resolve();
    }
  };

  const rejectShellSnapshotAfterResubscribeWaiters = (error: unknown) => {
    for (const waiter of shellSnapshotAfterResubscribeWaiters) {
      shellSnapshotAfterResubscribeWaiters.delete(waiter);
      waiter.reject(error);
    }
  ...
  const waitForShellSnapshotAfterResubscribe = (afterGeneration: number) => {
    if (shellSnapshotAfterResubscribeGeneration > afterGeneration) {
    ...
    return new Promise<void>((resolve, reject) => {
      shellSnapshotAfterResubscribeWaiters.add({
        afterGeneration,
        resolve,
        reject,
      });
  ...
  const unsubShell = input.client.orchestration.subscribeShell(
    (item) => {
      ...
      if (item.kind === "snapshot") {
        ...
        if (shellResubscribeGeneration > shellSnapshotAfterResubscribeGeneration) {
          shellSnapshotAfterResubscribeGeneration = shellResubscribeGeneration;
          resolveShellSnapshotAfterResubscribeWaiters();
        }
  ...
  const cleanup = () => {
    ...
    disposed = true;
    const disposedError = new EnvironmentConnectionDisposedError(environmentId);
    bootstrapGate.reject(disposedError);
    rejectShellSnapshotAfterResubscribeWaiters(disposedError);
    unsubShell();
    unsubTerminalEvent();
    unsubLifecycle();
    unsubConfig();
  ...
  return {
    ...
    reconnect: async () => {
      ...
      const resubscribeGenerationAtStart = shellResubscribeGeneration;
      const hadShellSnapshot = hasShellSnapshot;
      try {
        await input.client.reconnect();
        await input.refreshMetadata?.();
        if (hadShellSnapshot) {
          await waitForShellSnapshotAfterResubscribe(resubscribeGenerationAtStart);
        } else {
          await bootstrapGate.wait();
        }
      } catch (error) {
        bootstrapGate.reject(error);
        rejectShellSnapshotAfterResubscribeWaiters(error);
        throw error;
      }
    },
    dispose: async () => {
