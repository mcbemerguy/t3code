...
makeLayer()("037_ProjectionThreadSessionWorkflowRuns fresh database", (it) => {
  it.effect("adds workflow run storage on fresh databases", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;

      yield* runMigrations();

      const sessionColumns = yield* columnNames("projection_thread_sessions");
      assert.isTrue(sessionColumns.has("workflow_runs_json"));
      ...
      const rows = yield* sql<{ readonly workflowRunsJson: string }>`
        SELECT workflow_runs_json AS "workflowRunsJson"
        FROM projection_thread_sessions
        WHERE thread_id = 'thread-fresh-default'
      `;
      assert.strictEqual(rows[0]?.workflowRunsJson, "[]");
    }),
  );
});

makeLayer()("037_ProjectionThreadSessionWorkflowRuns existing database", (it) => {
