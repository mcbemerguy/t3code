...
import { ChatAttachment, OrchestrationMessageProviderDelivery } from "@t3tools/contracts";
...
function toProjectionThreadMessage(
  ...
  return {
    ...
    ...(row.providerDelivery !== null ? { providerDelivery: row.providerDelivery } : {}),
  };
}
...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const upsertProjectionThreadMessageRow = SqlSchema.void({
    ...
    execute: (row) => {
      ...
      const nextProviderDeliveryJson =
        row.providerDelivery !== undefined ? JSON.stringify(row.providerDelivery) : null;
      return sql`
        ...
        VALUES (
          ...
          COALESCE(
            ${nextProviderDeliveryJson},
            (
              SELECT provider_delivery_json
              FROM projection_thread_messages
              WHERE message_id = ${row.messageId}
            )
          ),
        ...
        DO UPDATE SET
          ...
          provider_delivery_json = COALESCE(
            excluded.provider_delivery_json,
            projection_thread_messages.provider_delivery_json
          ),
