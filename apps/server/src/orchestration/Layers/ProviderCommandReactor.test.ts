...
describe("ProviderCommandReactor", () => {
  ...
  async function createCustomAcpE2eHarness() {
    ...
    const layer = ProviderCommandReactorLive.pipe(
      ...
      Layer.provideMerge(
        makeProviderRegistryLayer([{ instanceId: customAcpPiLocalInstanceId }] as never),
      ),
      Layer.provideMerge(
        Layer.mock(GitWorkflowService)({
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          waitForUserMessageDelivered(harness.readModel, asMessageId("user-message-restricted-1")),
        );
