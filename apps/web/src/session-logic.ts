...
function isTerminalNonSuccessToolLifecycleStatus(
  status: WorkLogToolLifecycleStatus | undefined,
): boolean {
  return status === "failed" || status === "declined" || status === "stopped";
}

function mergeToolLifecycleStatus(
  previous: WorkLogToolLifecycleStatus | undefined,
  next: WorkLogToolLifecycleStatus | undefined,
): WorkLogToolLifecycleStatus | undefined {
  if (
    isTerminalNonSuccessToolLifecycleStatus(previous) &&
    (next === undefined || next === "completed")
  ) {
    return previous;
  }
...
function mergeDerivedWorkLogEntries(
  ...
  const toolLifecycleStatus = mergeToolLifecycleStatus(
    previous.toolLifecycleStatus,
    next.toolLifecycleStatus,
  );
  return {
    ...previous,
    ...next,
