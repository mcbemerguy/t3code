...
describe("createEnvironmentConnection", () => {
  ...
  it("keeps reconnect waiting when shell resubscribe starts after reconnect awaits the gate", async () => {
    const environmentId = EnvironmentId.make("env-1");
    const { client, emitShellResubscribe, emitShellSnapshot } = createTestClient();

    vi.mocked(client.reconnect).mockResolvedValueOnce(undefined);
    ...
    await expect(
      Promise.race([
        reconnectPromise.then(() => "resolved" as const),
