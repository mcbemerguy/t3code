...
describe("deriveWorkLogEntries", () => {
  ...
  it("keeps no-turn ACP lifecycle rows derivable", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "acp-cancelled-without-turn",
        kind: "tool.completed",
        ...
        payload: {
          ...
          status: "stopped",
          data: {
            kind: "pi.workflow.abort",
            acpTitle: "Abort workflow",
          },
        },
      }),
    ];
  ...
  it("preserves terminal non-success lifecycle status when a bare completion collapses into it", () => {
    for (const status of ["failed", "declined", "stopped"] as const) {
      const activities: OrchestrationThreadActivity[] = [
        makeActivity({
          id: `tool-${status}-update`,
          kind: "tool.updated",
          summary: "Workflow abort",
          tone: "tool",
          sequence: 1,
          payload: {
            itemType: "dynamic_tool_call",
            status,
            detail: "Abort requested",
            data: { toolCallId: `call-${status}` },
          },
        }),
        makeActivity({
          id: `tool-${status}-complete`,
          kind: "tool.completed",
          summary: "Workflow abort completed",
          tone: "tool",
          sequence: 2,
          payload: {
            itemType: "dynamic_tool_call",
            detail: "Abort completed",
            data: { toolCallId: `call-${status}` },
          },
        }),
      ];
      ...
      const [entry] = deriveWorkLogEntries(activities);
      expect(entry?.toolLifecycleStatus).toBe(status);
      expect(entry ? workEntryIndicatesToolSuccess(entry) : undefined).toBe(false);
    }
  });
