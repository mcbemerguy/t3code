...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("does not group adjacent work entries from different turns when expanded", () => {
    const rows = deriveMessagesTimelineRows({
      timelineEntries: [
        {
          id: "entry-work-1",
  ...
  it("keeps upstream changed-file work rows and review user rows in timeline order when expanded", () => {
    const assistantTurnDiffSummary = {
    ...
    const rows = deriveMessagesTimelineRows({
      ...
      expandedTurnIds: new Set(["turn-review" as never]),
      isWorking: false,
      activeTurnStartedAt: null,
      turnDiffSummaryByAssistantMessageId: new Map([
  ...
  it("preserves custom ACP/Pi thought and compact tool metadata when grouping expanded rows", () => {
    const rows = deriveMessagesTimelineRows({
      timelineEntries: [
        {
          id: "pi-thought-entry",
      ...
      expandedTurnIds: new Set(["turn-pi" as never]),
      isWorking: false,
      activeTurnStartedAt: null,
      turnDiffSummaryByAssistantMessageId: new Map(),
      revertTurnCountByUserMessageId: new Map(),
