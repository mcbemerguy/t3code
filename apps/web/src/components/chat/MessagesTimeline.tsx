    ...
    <div className="group flex flex-col items-end gap-1">
      ...
      <div className="flex w-full max-w-[80%] items-center justify-end pe-1 text-xs tabular-nums opacity-0 transition-opacity duration-200 focus-within:opacity-100 group-hover:opacity-100">
        <div className="flex shrink-0 items-center gap-2">
          {deliveryPending ? (
            <span className="text-muted-foreground/65">Delivering to provider...</span>
          ) : null}
          <Tooltip>
...
const WorkGroupSection = memo(function WorkGroupSection({
  ...
  const compactableToolEntries = onlyToolEntries
    ? nonEmptyEntries.filter((entry) => entry.tone !== "thinking")
    : [];
  const hasOverflow = compactableToolEntries.length > MAX_VISIBLE_WORK_LOG_ENTRIES;
  const visibleCompactableToolEntries =
    hasOverflow && !isExpanded
      ? new Set(compactableToolEntries.slice(-MAX_VISIBLE_WORK_LOG_ENTRIES))
      : null;
  const visibleEntries = visibleCompactableToolEntries
    ? nonEmptyEntries.filter(
        (entry) => entry.tone === "thinking" || visibleCompactableToolEntries.has(entry),
      )
    : nonEmptyEntries;
  const hiddenCount = Math.max(0, compactableToolEntries.length - MAX_VISIBLE_WORK_LOG_ENTRIES);
  const toolCallCount = compactableToolEntries.length;
  const groupLabel =
    onlyToolEntries && toolCallCount > 0
      ? toolCallCount === 1
        ? "1 tool call"
...
function workEntryIconName(workEntry: TimelineWorkEntry): WorkEntryIconName {
  ...
  const kind = workEntry.toolKind?.toLowerCase();
  const identity = normalizedToolIdentity(workEntry);
  if (kind === "read" || identity === "read" || identity === "read file") return "eye";
  if (kind === "search" || kind === "fetch") return "globe";
  if (identity === "write") return "hammer";
