...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const composerSendState = useMemo(
    ...
    [composerImages.length, composerTerminalContexts, prompt, sendBlockedReason],
  );
  ...
  const composerMenuItems = useMemo<ComposerCommandItem[]>(() => {
    ...
    if (composerTrigger.kind === "slash-command") {
      const builtInSlashCommandItems = [
        ...
        {
          id: "slash:workflow-abort",
          type: "slash-command",
          command: "workflow-abort",
          label: "/workflow-abort",
          description: "Abort a Pi workflow run by control command",
        },
  ...
  const showPlanSidebarToggle = Boolean(
    activePlan || sidebarProposedPlan || hasActiveWorkflowRuns || planSidebarOpen,
  );
  const composerFooterActionLayoutKey = useMemo(() => {
    ...
    return `idle:${composerSendState.hasSendableContent}:${isSendBusy}:${isConnecting}:${isPreparingWorktree}:${sendBlockedReason ?? ""}`;
  }, [
    activePendingIsResponding,
    activePendingProgress,
  ...
  const onSelectComposerItem = useCallback(
    (item: ComposerCommandItem) => {
      ...
      if (item.type === "slash-command") {
        ...
        if (item.command === "workflow-abort") {
          const replacement = "/workflow-abort ";
          const replacementRangeEnd = extendReplacementRangeForTrailingSpace(
            snapshot.value,
            trigger.rangeEnd,
            replacement,
